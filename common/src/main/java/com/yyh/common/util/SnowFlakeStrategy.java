...
public class SnowFlakeStrategy {
    ...
    /**
     * Math.abs(Integer.MIN_VALUE) 仍旧是Integer.MIN_VALUE，所以+1。
     * */
    private static long machineId = Math.abs(SystemEnvParams.getMacAddress().hashCode()+1) % (int) MAX_MACHINE_NUM; // 机器标识
    ...
    public SnowFlakeStrategy(long datacenterId, long machineId) {
        ...
        synchronized (SnowFlakeStrategy.class) {
